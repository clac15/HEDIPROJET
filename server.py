...
def create_boilerplate(directory):
    """
    Create a directory structure for a new website
    directory: path to the directory to be created and initialized with required files/directories
    """
    subdirs = ['controleurs', 'templates', 'model', 'static', path.join('static', 'img'), path.join('static', 'css'),]
    if path.isdir(directory):
        return False
    os.mkdir(directory)
    for subdir in subdirs:
        os.mkdir(path.join(directory, subdir))
    with open(path.join(directory, 'init.py'), 'w') as file:
        file.write("\"\"\"\nFicher initialisation (eg, constantes chargées au démarrage dans la session)\n\"\"\"")
    with open(path.join(directory, 'routes.toml'), 'w') as file:
        file.write("# Définition d'un tableau de routes au format TOML (https://toml.io/)\n# - url : chemin dans l'URL\n# - controleur : chemin vers le fichier du controleur\n# - template : chemin vers le fichier de template")
    return True
...
if __name__ == '__main__':
    ...
    parser.add_argument('-b', '--boilerplate', action='store_true')
    ...
    if args.boilerplate:  # special option to create a new empty website (does not run server)
        success = create_boilerplate(args.directory)
        if success:
            logger.info(f"A website boilerplate has been created in {args.directory}")
        else:
            logger.error(f"Directory {args.directory} already exists: cannot create boilerplate.")
        sys.exit(0)
