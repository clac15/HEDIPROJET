...
def delete_equipe(connexion, id_equipe):
    """
    Supprime une équipe et toutes ses associations
    """
    try:
        # 1. Supprimer les associations partie-équipe
        query1 = "DELETE FROM morpion.Partie_Equipe WHERE idEquipe = %s"
        execute_insert_update_delete(connexion, query1, [id_equipe])
        
        # 2. Supprimer les associations morpion-équipe
        query2 = "DELETE FROM morpion.Morpion_Equipe WHERE idEquipe = %s"
        execute_insert_update_delete(connexion, query2, [id_equipe])
        
        # 3. Supprimer l'équipe
        query3 = "DELETE FROM morpion.Equipe WHERE idEquipe = %s"
        return execute_insert_update_delete(connexion, query3, [id_equipe])
...
def create_partie(connexion, id_equipe1, id_equipe2, taille_grille, max_tours):
    ...
    Retourne (id_partie, id_journal) ou (None, None) si erreur
    """
    try:
        timestamp = datetime.now().strftime('%Y%m%d%H%M%S')
